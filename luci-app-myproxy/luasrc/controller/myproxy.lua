...
function index()
	...
	entry({"admin", "services", appname, "dnslist"}, cbi(appname .. "/client/dnslist"),_("DNS List"),3).leaf = true
...
function reset_config()
	luci.sys.call('/etc/init.d/myproxy stop')
	luci.sys.call('[ -f "/usr/share/myproxy/0_default_config" ] && cp -f /usr/share/myproxy/0_default_config /etc/config/myproxy')
	luci.http.redirect(api.url())
end

function show_menu()
	luci.sys.call("touch /etc/config/myproxy_show")
...
function hide_menu()
	luci.sys.call("rm -rf /etc/config/myproxy_show")
	luci.sys.call("rm -rf /tmp/luci-*")
	luci.sys.call("/etc/init.d/rpcd restart >/dev/null")
...
function link_add_node()
	...
	luci.sys.call("lua /usr/share/myproxy/subscribe.lua add log")
end
	...
	if key and key ~= "" then
		for k, e in ipairs(api.get_valid_nodes()) do
			if e.node_type == "normal" and e["remark"]:find(key) then
				luci.sys.call(string.format("uci -q del_list myproxy.@auto_switch[0].node='%s' && uci -q add_list myproxy.@auto_switch[0].node='%s'", e.id, e.id))
			end
		end
	end
	...
	if key and key ~= "" then
		for k, e in ipairs(ucic:get(appname, "@auto_switch[0]", "node") or {}) do
			if e and (ucic:get(appname, e, "remarks") or ""):find(key) then
				luci.sys.call(string.format("uci -q del_list myproxy.@auto_switch[0].node='%s'", e))
			end
		end
	end
	luci.http.redirect(api.url("auto_switch"))
...
function get_now_use_node()
	local e = {}
	local data, code, msg = nixio.fs.readfile("/tmp/etc/myproxy/id/global")
	if data then
...
function get_redir_log()
	...
	if nixio.fs.access("/tmp/etc/myproxy/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/myproxy/" .. id .. ".log")
...
function get_log()
	-- luci.sys.exec("[ -f /tmp/log/myproxy.log ] && sed '1!G;h;$!d' /tmp/log/myproxy.log > /tmp/log/myproxy_show.log")
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/myproxy.log' ] && cat /tmp/log/myproxy.log"))
end

function clear_log()
	luci.sys.call("echo '' > /tmp/log/myproxy.log")
end
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/myproxy/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		if use_time:find("%.") then
...
function set_node()
	...
	ucic:commit(appname)
	luci.sys.call("/etc/init.d/myproxy restart > /dev/null 2>&1 &")
...
function delete_select_nodes()
	...
	string.gsub(ids, '[^' .. "," .. ']+', function(w)
		for k, v in ipairs(auto_switch_node_list) do
			if v == w then
				luci.sys.call(string.format("uci -q del_list myproxy.@auto_switch[0].node='%s'", w))
			end
		end
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/myproxy/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function server_user_log()
	local id = luci.http.formvalue("id")
	if nixio.fs.access("/tmp/etc/myproxy_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/myproxy_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		luci.http.write(content)
	else
		luci.http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/myproxy_server.log' ] && cat /tmp/log/myproxy_server.log"))
end
