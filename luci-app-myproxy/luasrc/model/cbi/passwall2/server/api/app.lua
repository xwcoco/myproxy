...
local CONFIG = "myproxy_server"
local CONFIG_PATH = "/tmp/etc/" .. CONFIG
local LOG_APP_FILE = "/tmp/log/" .. CONFIG .. ".log"
local TMP_BIN_PATH = CONFIG_PATH .. "/bin"
local require_dir = "luci.model.cbi.myproxy.server.api."

local ipt_bin = sys.exec("echo -n $(/usr/share/myproxy/iptables.sh get_ipt_bin)")
local ip6t_bin = sys.exec("echo -n $(/usr/share/myproxy/iptables.sh get_ip6t_bin)")
