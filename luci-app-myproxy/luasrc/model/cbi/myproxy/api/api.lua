module("luci.model.cbi.myproxy.api.api", package.seeall)
fs = require "nixio.fs"
...
function get_bin_version_cache(file, cmd)
    sys.call("mkdir -p /tmp/etc/myproxy_tmp")
    if fs.access(file) then
        chmod_755(file)
        local md5 = sys.exec("echo -n $(md5sum " .. file .. " | awk '{print $1}')")
        if fs.access("/tmp/etc/myproxy_tmp/" .. md5) then
            return sys.exec("echo -n $(cat /tmp/etc/myproxy_tmp/%s)" % md5)
        else
            local version = sys.exec(string.format("echo -n $(%s %s)", file, cmd))
            if version and version ~= "" then
                sys.call("echo '" .. version .. "' > " .. "/tmp/etc/myproxy_tmp/" .. md5)
                return version
            end
        end
    end
