module("luci.model.cbi.myproxy.server.api.v2ray", package.seeall)
...
function gen_config(user)
    ...
    if user.transit_node and user.transit_node ~= "nil" then
        local transit_node_t = uci:get_all("myproxy", user.transit_node)
        ...
        local outbound = require("luci.model.cbi.myproxy.api.gen_v2ray").gen_outbound(transit_node_t, "transit")
        if outbound then
    ...
    local config = {
        log = {
            -- error = "/tmp/etc/myproxy_server/log/" .. user[".name"] .. ".log",
